...
const intlMessages = defineMessages({
  moreMessages: {
    ...
    description:
      'Chat message when the user has unread messages below the scroll',
  },
  emptyLogLabel: {
...
class TimeWindowList extends PureComponent {
  ...
  componentDidUpdate(prevProps) {
    ChatLogger.debug(
      'TimeWindowList::componentDidUpdate',
      { ...this.props },
      { ...prevProps }
    );
    if (this.virualRef) {
  ...
  handleScrollUpdate(position, target) {
    ...
    if (
      position !== null &&
      position + target?.offsetHeight === target?.scrollHeight
    ) {
  ...
  rowRender({ index, parent, style, key }) {
    const { id, timeWindowsValues, dispatch, chatId } = this.props;
  ...
  renderUnreadNotification() {
    const { intl, count, timeWindowsValues } = this.props;
    const { userScrolledBack } = this.state;
  ...
  render() {
    const { timeWindowsValues, width } = this.props;
    const { scrollArea, scrollPosition, userScrolledBack } = this.state;
    ChatLogger.debug(
      'TimeWindowList::render',
      { ...this.props },
      { ...this.state },
      new Date()
    );
