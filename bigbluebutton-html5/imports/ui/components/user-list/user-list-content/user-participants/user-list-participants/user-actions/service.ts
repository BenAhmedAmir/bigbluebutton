...
export const isVoiceOnlyUser = (userId: string) =>
  userId.toString().startsWith('v_');
...
export const generateActionsPermissions = (
  ...
  const allowedToResetStatus =
    hasAuthority && subjectUser.emoji !== EMOJI_STATUSES.none && !isDialInUser;
  ...
  const allowedToChangeWhiteboardAccess =
    currentUser.presenter && !amISubjectUser;

  const allowedToEjectCameras =
    amIModerator && !amISubjectUser && usersPolicies.allowModsToEjectCameras;

  const allowedToSetPresenter =
    amIModerator && !subjectUser.presenter && !isDialInUser;
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore - temporary, while meteor exists in the project
